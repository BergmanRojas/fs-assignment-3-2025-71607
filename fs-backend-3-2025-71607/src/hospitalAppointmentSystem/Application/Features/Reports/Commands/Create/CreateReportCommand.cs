...
public class CreateReportCommand : IRequest<CreatedReportResponse>,  ILoggableRequest, ITransactionalRequest
{
    ...
    public class CreateReportCommandHandler : IRequestHandler<CreateReportCommand, CreatedReportResponse>
    {
        ...
        {
            // Soft delete uygulanm�� raporlar� kontrol et
            var deletedReport = await _reportRepository.GetAsync(r => r.AppointmentID == request.AppointmentID && r.DeletedDate != null);

            if (deletedReport != null)
            {
                // Silinmi� rapor varsa geri y�kle
                deletedReport.DeletedDate = null;
                deletedReport.Text = request.Text; // Gerekirse yeni rapor i�eri�ini g�ncelleyin
            ...
            {
                // E�er silinmi� bir rapor yoksa, yeni raporu ekle
